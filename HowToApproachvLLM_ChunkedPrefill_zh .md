...
**`get_cache_hit_tokens(seq)`***
- 尝试寻找已缓存的 block (前缀缓存 prefix caching)
- 返回命中缓存的block 以及 命中缓存的token数

**`allocate(seq, num_new_tokens, num_new_computed_tokens, new_computed_blocks)`：**
- num_new_tokens:当前序列调度的token数, num_new_computed_tokens:当前序列命中缓存的token数, new_computed_blocks:当前序列命中缓存的block id
- 分配该序列命中缓存的block: 命中缓存的block `ref_cout` + 1
- 分配该序列未命中缓存的block: 创建新的 block 或者 从当前未满的block中分配
- 返回 True/False  表示是否分配成功
...
**核心技术点: ** continuous batching、 chunked_prefill、 prefix caching 

### 5.1 核心设计

**两类队列：**
1. **Waiting 队列**：尚未开始的新序列(存在WAITING状态的序列和PREEMETED状态的序列, 这两个序列都是没有执行的序列)
2. **Running 队列**：正在运行的序列(可能同时存在 prefill阶段的序列 和 decode阶段的序列)
...
调度器 优先执行running队列中的序列

**调度流程: **
1. **首先调度 running 队列中的序列**
  - 检查 running 队列中是否有序列需要分配Block 且 是否有足够的剩余调度token配额以及序列数量配额
  - 依次从 running 队列队首取出序列
  - 计算该序列本次需要调度的token数(num_new_tokens)  计算逻辑为:num_new_tokens = (prompt tokens数 + completion tokens数) - computed tokens数
  - 对本次需要调度的token数(num_new_tokens) 应用chunked_prefill阈值和调度token阈值.
  - 尝试循环为本次需要调度的token分配Block
  - 若分配Block失败 则循环从 running 队列队尾 **抢占(preempt)** 优先级最低的序列, 直到分配成功或者空间不足退出running调度流程.
  - 若分配Block成功 则跳出分配block的循环并将该序列添加到本次需要调度的集合中, 并进行下一个序列的循环遍历, 直到达到开始的边界条件阈值

2. **检查调度 running 队列时是否有执行抢占(preempt)**
  - 若有被抢占的序列, 此时说明没有可分配Block的空间, 则直接不进入下一步 waiting 队列的调度
  - 若没有被抢占的序列, 则进入下一步的 waiting 队列的调度

3. **开始调度 waiting 队列中的序列 **
  - 检查 waiting 队列中是否有序列需要分配Block 且 是否有足够的剩余调度token配额以及序列数量配额
  - 尝试寻找已缓存的 Block, 并根据寻找结果 来计算该序列本次需要调度的token数(num_new_tokens) 计算逻辑为: num_new_tokens = num tokens数 - cache_hit_tokens数
  - 对本次需要调度的token数(num_new_tokens) 应用chunked_prefill阈值和调度token阈值.
  - 尝试为本次需要调度的token分配Block
  - 若分配Block失败 则将该序列添加到 **skipped_waiting_seqs** 队列中, 表示跳过该序列
  - 若分配Block成功 则将该序列添加到本次需要调度的集合中, 并进行下一个序列的循环遍历, 直到达到开始的边界条件阈值
...
**生成之后：**
- 检查序列是否属于chunked_prefill中间阶段(是否完成整个prefill的推理)
- 若属于: 不向该序列添加生成的token
- 若不属于: 正常添加生成的token
